...
import { Mail, ArrowRight, Loader2, CheckCircle2, AlertCircle, Smartphone, Lock, X, Fingerprint } from "lucide-react";
...
export function LoginForm({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // In production, you'd check if the email exists in your database
            // For now, we'll randomly decide (in production, this would be based on actual data)
            const isNewUser = Math.random() > 0.5; // Replace with actual API check

            // Redirect to email-login page with query parameters
    ...
    return (
        <div className="w-full max-w-[440px] mx-auto px-4 z-20 relative">
            ...
            {/* NO MODAL CONTAINER - Floating Elements Strategy */}
            ...
            {/* Social Stack - Vertical list of labeled buttons */}
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
            ...
            >
                ...
                <button
                    ...
                    className="w-full bg-white hover:bg-gray-100 text-black font-semibold h-14 rounded-full flex items-center justify-center gap-4 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-lg group"
                >
                ...
                {/* Other Options - Glass Effect */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button
                        onClick={() => window.location.href = '/api/auth/x'}
                        className="bg-black/40 hover:bg-black/60 backdrop-blur-md border border-white/10 hover:border-white/30 text-white h-14 rounded-full flex items-center justify-center gap-3 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] group"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <button
                        onClick={() => router.push('/passkey-login')}
                        className="bg-black/40 hover:bg-black/60 backdrop-blur-md border border-white/10 hover:border-white/30 text-white h-14 rounded-full flex items-center justify-center gap-3 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] group"
                    >
            ...
            {/* Manual Entry - Floating Glass Input */}
            <motion.form
